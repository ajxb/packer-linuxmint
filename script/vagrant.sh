...
if [ "$INSTALL_VAGRANT_KEY" = "true" ] || [ "$INSTALL_VAGRANT_KEY" = "1" ]; then
    ...
    # If the docker group exists, add vagrant user to it.
    dckGrp="docker"
    if grep -q "^${dckGrp}:" /etc/group; then
        echo "==> Add vagrant user to '${dckGrp}' group"
        /usr/sbin/usermod -aG ${dckGrp} vagrant
    fi
